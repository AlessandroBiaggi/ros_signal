...
TEST_F(SyncPolicyEpsilonTimeTest1, emplace) {
    ...
    sync_policy_.emplace<0>(storage_, stamp, message);

    EXPECT_FALSE(std::get<0>(storage_.entry_queues_).empty());
    EXPECT_EQ(std::get<0>(storage_.entry_queues_).back(), ros_signal::Entry<int>(stamp, message));
}
 */
...
TEST_F(SyncPolicyEpsilonTimeTest1, try_sync) {
    ...
    EXPECT_EQ(*std::get<0>(*out), ros_signal::Entry<int>(stamp, message));
}
...
using SyncPolicyEpsilonTimeTest4 = SyncPolicyEpsilonTimeTest<int, Message, HeaderMessage, StampMessage>;
TEST_F(SyncPolicyEpsilonTimeTest4, mixed_entries_1) {
    ...
    auto entry2 = MessageEntry(stamp);
    auto entry3 = HeaderMessageEntry();
    auto entry4 = StampMessageEntry();
...
using SyncPolicyEpsilonTimeTest5 = SyncPolicyEpsilonTimeTest<
    ...
    HeaderMessage,
    std::shared_ptr<HeaderMessage>,
    StampMessage,
    std::shared_ptr<StampMessage>
>;
TEST_F(SyncPolicyEpsilonTimeTest5, mixed_entries_2) {
    ...
    auto entry3 = MessageEntry(stamp);
    auto entry4 = MessageSharedPtrEntry(stamp, std::make_shared<Message>());
    auto entry5 = HeaderMessageEntry();
    auto entry6 = HeaderMessageSharedPtrEntry(std::make_shared<HeaderMessage>());
    auto entry7 = StampMessageEntry();
    auto entry8 = StampMessageSharedPtrEntry(std::make_shared<StampMessage>());
