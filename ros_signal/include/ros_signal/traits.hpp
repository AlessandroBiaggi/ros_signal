...
namespace ros_signal {
    ...
    struct header<T, std::enable_if_t<detail::is_header_v<decltype(std::declval<T>().header)> > > {
        using message_type = std::decay_t<T>;
        using type = decltype(std::declval<message_type>().header);

        static const type &get(const message_type &msg) {
            return msg.header;
        }

        static type &get(message_type &msg) {
            return msg.header;
        }
    };
    ...
    struct stamp<T, std::enable_if_t<detail::is_time_v<decltype(std::declval<T>().stamp)> > > {
        using message_type = std::decay_t<T>;
        using type = decltype(std::declval<message_type>().stamp);
    ...
    struct stamp<T, std::enable_if_t<has_header_v<T> > > {
        using message_type = std::decay_t<T>;
        using type = decltype(header<message_type>::get(std::declval<message_type>()).stamp);
