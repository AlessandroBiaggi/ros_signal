...
namespace ros_signal {
    ...
    public:
        ...
        virtual void signal(const rclcpp::Time &stamp, const message_type &msg);

        void signal(const message_type &msg) {
            signal(ros_signal::get_stamp(msg), msg);
        }

        virtual bool check(const message_type &msg) const;
    ...
    private:
        ...
        using error_it = typename decltype(errors_)::iterator;
        std::unique_ptr<ros_signal::detail::CheckImplBase<message_type, error_it>> checks_;
    ...
    SignalMonitorBase<MsgT, AllocT>::SignalMonitorBase(
        ...
        alloc_(alloc),
        can_unlatch_(false),
        checks_(std::make_unique<ros_signal::detail::CheckImpl<message_type, error_it, CheckTs...>>(
            std::move(checks)...)) {
    }

    template<typename MsgT, typename AllocT>
	void SignalMonitorBase<MsgT, AllocT>::signal(const rclcpp::Time &stamp, const message_type &msg) {
    ...
    template<typename MsgT, typename AllocT>
    bool SignalMonitorBase<MsgT, AllocT>::check(const message_type &msg) const {
