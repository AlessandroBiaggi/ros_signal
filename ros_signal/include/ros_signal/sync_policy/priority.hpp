...
namespace ros_signal::sync_policy {
    ...
    template<typename... MsgTs>
    template<std::size_t I, typename AllocT>
    void Priority<MsgTs...>::push(storage_type<AllocT> &storage, const entry_type<message_type<I> > &entry) const {
    ...
    template<typename... MsgTs>
    template<std::size_t I, typename AllocT>
    void Priority<MsgTs...>::push(storage_type<AllocT> &storage, entry_type<message_type<I> > &&entry) const {
        static_assert(0 <= I && I < message_count);
                ...
                if (rclcpp::Time(eq.front().stamp()) <= min_time) {
                    ...
                    if (min_time < rclcpp::Time(eq.back().stamp())) {
                        upper_bound = std::upper_bound(
                            eq.begin(), eq.end(), min_time, LessEntryStamp<message_type<I> >());
                    }
