...
namespace ros_signal::sync_policy {
    ...
    public:
        ...
        using message_type = std::tuple_element_t<I, std::tuple<typename entry_type<MsgTs>::message_type...> >;
         ...
         * @post Given `out = sync(...)`, either `out == std::nullopt` or `a.stamp() - b.stamp() <= epsilon()` for each `a`, `b` in `out` such that `!entry_type<message_type<I>>::is_null(a) && !entry_type<message_type<I>>::is_null(b)`
         */
         ...
         * @pre Entries of type `entry_type<message_type<I>>` in `storage` are sorted by `less_entry_time<message_type<I>>`
         * @post Entries of type `entry_type<message_type<I>>` in `storage` are sorted by `less_entry_time<message_type<I>>`
        ...
        void push(storage_type<AllocT> &storage, const entry_type<message_type<I> > &entry) const;
         ...
         * @param storage message storage
         * @param entry entry to push
         * @pre Entries of type `entry_type<message_type<I>>` in `storage` are sorted by `entry_compare_type<message_type<I>>`
         * @post Entries of type `entry_type<message_type<I>>` in `storage` are sorted by `entry_compare_type<message_type<I>>`
         * @post if `storage.enabled<I>()` then `storage.count()` is incremented by one
         * @post if `storage.enabled<I>()` then `storage.count<I>()` is incremented by one
         * @post if `storage.enabled<I>()` then `!storage.empty<I>()`
         * @note if exceptions are thrown, this function has no effect on the storage
         ...
         */
        template<std::size_t I, typename AllocT>
        void push(storage_type<AllocT> &storage, entry_type<message_type<I> > &&entry) const;
         ...
         * @pre Entries of type `entry_type<message_type<I>>` in `storage` are sorted by `entry_compare_type<message_type<I>>` for each `I` in `[0, message_count)`
         * @post Entries of type `entry_type<message_type<I>>` in `storage` are sorted by `entry_compare_type<message_type<I>>` for each `I` in `[0, message_count)`
         * @post `up_to < e.stamp()`, for each entry `e` of type `entry_type<message_type<I>>` in `storage`, for each index `I` in `[0, message_count)`
         ...
         * @tparam AllocT message storage allocator type
         * @param storage message storage
         * @pre Entries of type `entry_type<J>` in `storage` are sorted by `entry_compare_type<message_type<J>>` for each `J` in `[I, message_count)`
         * @post Entries of type `entry_type<J>` in `storage` are sorted by `entry_compare_type<message_type<J>>` for each `J` in `[I, message_count)`
    ...
    private:
         ...
         * @pre messages in `std::get<J>(storage)` are sorted by `entry_compare_type<message_type<J>>` for each `J` in `[I, message_count)`
         * @pre `!empty<J>()` for each `J` in `[I, message_count)`
         * @pre `min <= max`
         * @post Entries of type `entry_type<J>` are sorted by `entry_compare_type<message_type<J>>` for each `J` in `[I, message_count)`
         ...
         */
        template<std::size_t I = 0, typename AllocT>
        rclcpp::Time get_max_stamp_helper(storage_type<AllocT> &storage) const;
         ...
         * @param up_to maximum time for an entry to be cleared
         * @pre Entries of type `entry_type<J>` in `storage` are sorted by `entry_compare_type<message_type<J>>` for each `J` in `[I, message_count)`
         * @post Entries of type `entry_type<J>` in `storage` are sorted by `entry_compare_type<message_type<J>>` for each `J` in `[I, message_count)` (1)
    ...
    template<typename... MsgTs>
    template<std::size_t I, typename AllocT>
    void EpsilonTime<MsgTs...>::push(storage_type<AllocT> &storage, const entry_type<message_type<I> > &entry) const {
        ...
        if (storage.template enabled<I>()) {
            ...
            if (eq.empty() || rclcpp::Time(eq.back().stamp()) <= rclcpp::Time(entry.stamp())) {
                ...
                auto pos = std::upper_bound(eq.begin(), eq.end(), entry, LessEntryStamp<message_type<I> >());
                eq.insert(pos, entry);
            }
        }
        ...
        ros_signal_assert((storage.template enabled<I>() && pre_count_I + 1 == post_count_I && !storage.template empty<I>())
            || pre_count_I == post_count_I);
    }

    template<typename... MsgTs>
    template<std::size_t I, typename AllocT>
    void EpsilonTime<MsgTs...>::push(storage_type<AllocT> &storage, entry_type<message_type<I> > &&entry) const {
        static_assert(0 <= I && I < message_count);
        ...
        if (storage.template enabled<I>()) {
            ...
            if (eq.empty() || rclcpp::Time(eq.back().stamp()) <= rclcpp::Time(entry.stamp())) {
                eq.push_back(std::forward<entry_type<message_type<I> > >(entry));
            } else {
                auto pos = std::upper_bound(eq.begin(), eq.end(), entry, LessEntryStamp<message_type<I> >());
                eq.insert(pos, std::forward<entry_type<message_type<I> > >(entry));
            }
        }
    ...
    void EpsilonTime<MsgTs...>::emplace(
        ...
        if (storage.template enabled<I>()) {
            ...
            if (eq.empty() || rclcpp::Time(eq.back().stamp()) <= stamp) {
                ...
                auto pos = std::upper_bound(eq.begin(), eq.end(), stamp, LessEntryStamp<message_type<I> >());
    ...
    std::size_t EpsilonTime<MsgTs...>::clear(storage_type<AllocT> &storage, const rclcpp::Time &up_to) const {
        ...
        auto pos = std::upper_bound(eq.begin(), eq.end(), up_to, LessEntryStamp<message_type<I> >());
    ...
    auto EpsilonTime<MsgTs...>::try_sync(storage_type<AllocT> &storage) const -> std::optional<output_type> {
        ...
        if (storage.any_enabled_helper(index_sequence)) {
            while (!(storage.empty() || out.has_value())) {
                ...
                const auto min_time = max_time - epsilon_;
                 ...
                 */
                if (try_sync_helper(storage, min_time, max_time)) {
        ...
        if constexpr (I < message_count) {
            ...
            if (storage.template enabled<I>()) {
                ...
                if (const auto &b = eq.back(); rclcpp::Time(b.stamp()) <= max) {
                    ...
                    auto upper_bound = std::upper_bound(eq.begin(), eq.end(), max, LessEntryStamp<message_type<I> >());
            ...
            ros_signal_assert(storage.template is_sorted_helper<I>());
            ros_signal_assert(!(storage.template enabled<I>() && sync && storage.template empty<I>()));
            const auto post_count = storage.template count<I>();
            ros_signal_assert(pre_count - erased == post_count);
    ...
    template<typename... MsgTs>
    template<std::size_t I, typename AllocT>
    rclcpp::Time EpsilonTime<MsgTs...>::get_max_stamp_helper(storage_type<AllocT> &storage) const {
        static_assert(0 <= I && I < message_count);

        if constexpr (I < message_count - 1) {
            ros_signal_assert(!storage.template empty<I>());

            auto max_stamp = get_max_stamp_helper<I + 1>(storage);
            if (storage.template enabled<I>()) {
                const auto stamp = rclcpp::Time(std::get<I>(storage.entry_queues_).front().stamp());
                if (max_stamp < stamp) {
                    max_stamp = stamp;
                }
            }
            ...
            return std::get<I>(storage.entry_queues_).front().stamp();
        }
    }
    ...
    public:
         ...
         * @return `true` if the message storage has no entries of type `entry_type<message_type<I>>`, `false` otherwise
         */
        template<std::size_t I>
         ...
         * @tparam I index of the message type
         * @return `true` if messages of type `message_type<I>` are enabled, `false` otherwise
         */
        template<std::size_t I>
    ...
    private:
        ...
        [[nodiscard]] bool is_sorted_helper() const {
            ...
            return std::is_sorted(eq.begin(), eq.end(), LessEntryStamp<message_type<I> >());
        }
