...
namespace ros_signal::detail {
    ...
    public:
        ...
        virtual bool operator()(const MsgT &, IterT, bool) = 0;

        virtual bool operator()(const MsgT &, std::nullptr_t, bool) = 0;
    };
    ...
    public:
        ...
        static constexpr auto check_number = sizeof...(CheckTs);
        ...
        bool operator()(const MsgT &msg, IterT it, bool keep_latch) override {
            return check_impl<0, IterT>(msg, it, keep_latch);
        }

        bool operator()(const MsgT &msg, std::nullptr_t, bool keep_latch) override {
            return check_impl<0, std::nullptr_t>(msg, nullptr, keep_latch);
        }
    ...
    private:
        ...
        bool check_impl(const MsgT &msg, IterU it, bool);
    };

    template<typename MsgT, typename IterT, typename... CheckTs>
    template<std::size_t Idx, typename IterU>
    bool CheckImpl<MsgT, IterT, CheckTs...>::check_impl(const MsgT &msg, IterU it, bool keep_latch) {
        static_assert(std::is_same_v<IterU, IterT> || std::is_same_v<IterU, std::nullptr_t>);
        static_assert(0 <= Idx && Idx <= check_number);
        if constexpr (Idx < check_number) {
            const auto i_ok = std::invoke(std::get<Idx>(checks_), msg);
            const auto n_ok = [&] {
                if constexpr (std::is_same_v<IterT, IterU>) {
                    *it = !i_ok || *it && keep_latch;
                    return check_impl<Idx + 1>(msg, std::next(it), keep_latch);
                } else {
                    return check_impl<Idx + 1>(msg, nullptr, true);
                }
            }();
